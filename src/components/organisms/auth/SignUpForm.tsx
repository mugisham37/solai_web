...
export function SignUpForm() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      signIn({ email, name: name || undefined, onboardingComplete: false });
      router.push("/2fa-setup");
    }, 2200);
    return () => window.clearTimeout(id);
  }, [magicSent, email, name, router, signIn]);
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    signIn({ email: parsed.data.email, name: parsed.data.name, onboardingComplete: false });
    setLoading(false);
  ...
  const handlePasskeySignup = async (e: React.FormEvent) => {
    ...
    signIn({ email, name, onboardingComplete: false });
    setLoading(false);
    router.push("/verify-email");
  };
