...
export function WelcomeStep() {
  ...
  const { draft, ready, advanceWelcome } = useOnboardingDraft();

  useEffect(() => {
    if (ready && draft.currentStep > 0) {
      router.replace(STEP_ROUTES[draft.currentStep]);
    }
  }, [ready, draft.currentStep, router]);
